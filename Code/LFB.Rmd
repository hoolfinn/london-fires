...
```{r global}
  ...
  # create borough map
    london_shp <- readShapePoly("C:/Users/Finn/OneDrive/Data_Science/london-fires/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp")
    london_map <- fortify(london_shp)
    london_map <- rename(london_map, Easting_rounded = long, Northing_rounded = lat)
    
    # load fire data
    fires <- as_tibble(readRDS("C:/Users/Finn/OneDrive/Data_Science/london-fires/Data/fires2.rds"))
    fires$PropertyCategory <- factor(fires$PropertyCategory)
    # N <- nrow(fires)
    
    
    # load fire data
    map.data <- readRDS("C:/Users/Finn/OneDrive/Data_Science/london-fires/Data/mapData4.rds")
